...
- **`NEXT_PUBLIC_STYTCH_PUBLIC_TOKEN`**: Required for Stytch login. Without it, `app/providers.tsx` throws at module evaluation.
- **Environment variables**: The app hard-requires `NEXT_PUBLIC_CROSSMINT_CLIENT_API_KEY` and `NEXT_PUBLIC_STYTCH_PUBLIC_TOKEN`. Without valid keys, the app compiles but throws a client-side error in the React tree.
